...
const getAnalyticsData = async (propertyId, client, range = "weekly", customStart = null, customEnd = null) => {
    const analyticsData = googleapis_1.google.analyticsdata({ version: "v1beta", auth: client });
    ...
    const [report, devices, countries, sources, pages, realtime, trend, newReturning, browsers, operatingSystems,] = await Promise.all([
        // Core metrics
...
function formatAnalytics(data) {
    ...
    (data?.newReturning?.rows || []).forEach((row) => {
        ...
        if (type === "new" || type.startsWith("new"))
            newReturning.new += count;
        else
            newReturning.returning += count;
    });
    ...
    (data?.countries?.rows || []).forEach((row) => {
        const k = row.dimensionValues?.[0]?.value || "Unknown";
        if (k !== "(not set)")
            countries[k] = Number(row.metricValues?.[0]?.value || 0);
    });
    const trafficSources = {};
    (data?.sources?.rows || []).forEach((row) => {
        const s = row.dimensionValues?.[0]?.value || "Direct";
        if (s !== "(not set)")
            trafficSources[s] = Number(row.metricValues?.[0]?.value || 0);
    });
    ...
    rtRows.forEach((row) => {
        ...
        if (country !== "(not set)")
            rtByCountry[country] = (rtByCountry[country] || 0) + users;
        rtByDevice[device] = (rtByDevice[device] || 0) + users;
    });
    const sortDesc = (obj) => Object.entries(obj).map(([name, users]) => ({ name, users })).sort((a, b) => b.users - a.users);
    ...
    (data?.browsers?.rows || []).forEach((row) => {
        const k = row.dimensionValues?.[0]?.value || "Unknown";
        if (k !== "(not set)")
            browsers[k] = Number(row.metricValues?.[0]?.value || 0);
    });
    const operatingSystems = {};
    (data?.operatingSystems?.rows || []).forEach((row) => {
        const k = row.dimensionValues?.[0]?.value || "Unknown";
        if (k !== "(not set)")
            operatingSystems[k] = Number(row.metricValues?.[0]?.value || 0);
    });
    return {
